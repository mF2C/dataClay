...
public class AgentMockup {
	...
	if (isCloud) {
	    ...
	    // For the test it is read from the config file, considering static => leader, mobile => normal
	...
	registerExternalDataManagement(myLeaderDM);

	// Data operation: execute
	Agent myAgent = (Agent) Agent.getByAlias(myId);
	myAgent.federate(myLeaderDM.getName()); // myAgent (and Device, etc) has been replicated in the leader
	...
	// For this test I will read it from the config file since I need to control them
	// Normally, this would be generated by Discovery and stored in a configuration file
	try {
	...
	return new DataClayInstance(new DataClayInstanceID(id), id, hostname, port);
    }

    private static void registerExternalDataManagement(DataClayInstance externalInfo) {
	ClientManagementLib.registerExternalDataClay(externalInfo.getDcID(), externalInfo.getName(),
		externalInfo.getHost(), externalInfo.getPort());
    }
